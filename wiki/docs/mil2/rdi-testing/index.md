...
Every ECU in a vehicle holds engineering parameters — sensor readings, internal
software states, actuator feedbacks, learned values — that development,
production and aftersales all need to inspect from the outside. **Tester Data**
is the family of diagnostic commands that exposes them, and the **RDI (Readable
Diagnostic Identifier)** is its most-used element. This article covers what
DIDs are, how they are specified in the CDD, and how to design and execute a
proper RDI test.

Tester Data is used to:
...
These definitions exist so that the vehicle can be serviced at the **End of
Line (EOL)** in the plant, in **aftersales** workshops, and during development,
fleet and validation activities.

## What is a DID?

A **DID (Diagnostic Identifier)** identifies an engineering parameter that is
...
A DID is not just a debugging hook — it is a contractual obligation of the ECU:
...
- If a component is faulty, the DID must report the value **actually read by
  ...
  internally.
- If a parameter does not exist in a given application (variant or outfitting),
  the ECU must answer with a negative response meaning *"parameter not
  available"* — **NRC 0x31** (`requestOutOfRange`) — not crash or stay silent.
...
- **Mandatory DIDs** — required by diagnostic norms such as ISO 14229. These
  are the *identification parameters* (ECU identification, software versions,
  …) and generally carry identifiers in the **`F1xx`** range.
...
RDI testing runs over standard UDS services (see [Diagnosis](../diagnosis/index.md)
...
`XX XX` is the 2-byte RDI identifier; `YY ...` is the raw value returned by the
ECU, echoing the identifier first.
...
!!! tip "Always convert before judging"
    ...
    type. When a test result looks wrong, first check whether you are comparing
    raw hex against a physical expectation.

## DIDs in the CDD

The **CDD (CANdela Diagnostic Document)** is the ECU's complete diagnostic
specification, authored with **CANdelaStudio** (Vector). It is the single
...
1. **DID Overview** — the list of all DIDs across all variants: identifier,
   description, and whether each is implemented in Common Diagnostics and/or in
   specific variants.
2. **DID Implementation** — the per-DID detail screen (below): name and
   ...
   checkboxes enabling read and/or write access, the protocol services
   (`$22` / `$2E`), and the **DataRecord** table giving byte/bit position,
   name and data type of each field. The right-hand side shows the exact
   request, positive and negative response frames; greyed-out entries mean the
   service is not enabled (e.g. a read-only DID has `$2E` in grey).
...
RDI testing is the simplest diagnostic test to realize, and it runs in the
**default session** (`10 01`) — no extended session or security unlock needed.
...
A single positive answer proves the service exists — nothing more. The value
could be a **frozen constant**: a software bug, a stuck variable or a broken
scaling can all return a perfectly formatted but dead value. To say the test is
OK, you need **at least three readings** proving the RDI tracks the internal
variable:
...
The internal variable is forced with the development/debug tool chain for the
ECU under test; the RDI is read over UDS. If the diagnostic value follows the
...
A realistic RDI test request, as seen on a Stellantis diesel program:

- **ECU:** ECM (Engine Control Module)
- **Sub-system:** SCR (Selective Catalytic Reduction) aftertreatment
...
The ECU supplier answered with a multi-bit DID, **`0x2409` — "SCR System Dosing
Valve 1 Preconditions"**, where each bit of the DataRecord reports one enable
condition:
...
!!! success "Key takeaways"
    - A DID is a diagnostic identifier for an engineering parameter; **RDI** =
      readable (service `$22`), **WDI** = writeable (service `$2E`).
    - DIDs must be readable at key ON, engine running, any vehicle speed; on
      sensor faults they report the *real* sensor value; missing parameters get
      **NRC 0x31**.
    ...
    - A passing RDI test needs **at least three readings**: current value,
      forced minimum, forced maximum — one answer only proves the service
      exists.
    - Real-world pattern: bitmask DIDs like `0x2409` pack a component's test
      preconditions into a single read, decoded by DIAnalyzer from the CDD.

## Sub-sections

- [RDI Testing](rdi-testing/index.md)
...
- :material-file-pdf-box: [RDI Testing](../../assets/mil2/18_RDI_Testing/RDI_Testing.pdf) — PDF, 1.8 MB
