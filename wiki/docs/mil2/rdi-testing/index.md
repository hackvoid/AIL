...
Every Electronic Control Unit (ECU) in a vehicle holds engineering parameters
— sensor readings, internal software states, actuator feedback, learned values
— that development, production and aftersales teams all need to inspect from
the outside. **Tester Data** is the family of diagnostic commands that exposes
them, and the **Readable Diagnostic Identifier (RDI)** is the element used
most often. This article explains what a Diagnostic Identifier (DID) is, how
to find one in the ECU's diagnostic specification, and how to design and
execute a reliable RDI test.
...
link. Each DID is a numbered entry into the ECU's memory: the tester requests
a DID by number, and the ECU returns the value stored behind it.
...
exchanges below cover both RDI and WDI access.
...
!!! tip "Always convert before judging"
    A raw `62 24 09 01 20` has no meaning until you apply the DID's conversion
    formula. When a test result looks wrong, first check whether you are
    comparing raw hex against a physical expectation — a common cause of
    apparent RDI test failures.

## DIDs in the CDD

The **CANdela Diagnostic Document (CDD)** is the ECU's complete diagnostic
specification, authored with **CANdelaStudio** (Vector). It is the
authoritative reference for what to test: services, DIDs, data types, sessions and
...
**default session** (`10 01`) — no extended session or security unlock is
needed.
...
A single positive answer only proves the service exists. The value
could be a **frozen constant**: a software bug, a stuck variable or a broken
scaling can all return a correctly formatted but stale value. To pass the
test, you need **at least three readings** proving the RDI tracks the internal
variable:
...
conversion — is working, and the test can be reported as passed.
...
The following worked example shows a realistic RDI test request, as seen on a
...
key run reads False) — the other preconditions are met.
...
!!! success "Key takeaways"
    - A DID identifies an ECU engineering parameter exposed on the diagnostic
      link; **RDI** = readable (service `$22`), **WDI** = writeable (service
      `$2E`).
    - DIDs must be readable at key ON, engine running, any vehicle speed; on
      ...
      get **NRC 0x31**.
    - Mandatory identification DIDs use the `F1xx` range (ISO 14229);
    ...
    - A passing RDI test runs in the default session and requires **at least
      three** readings — current value, forced minimum, forced maximum.
