...
This is the first hands-on diagnostic exercise. Earlier lessons covered faults
and DTCs (Diagnostic Trouble Codes) in theory; here you will *cause* one on
purpose and observe how an ECU reacts, step by step. Missing-message supervision
is one of the most common scenarios in vehicle testing, and running it end to
end makes the DTC life cycle concrete: recognition, confirmation, healing, and
driving-cycle re-evaluation.
...
- document a test with screenshots, traces, and a reason for each transition.
...
The scenario: an ECU expects a cyclic CAN message that stops arriving.
...
ECUs actively supervise the messages they receive. The receiving software knows
each cyclic message's expected period, and a *supervision function* counts the
time since the last frame arrived. If nothing arrives within a timeout
(typically a multiple of the nominal cycle time, plus a *debounce* — also called
*maturation* — time defined in the DTC table), the fault is recognized.
...
The **status byte** returned by service `$19` encodes the fault's current state.
Each bit reports one aspect of the fault's life cycle. The ones that matter
here:
...
key-on to key-off — and a key-off is complete only after the ECU finishes its
...
| **CANalyzer** | Simulates the rest of the bus: you will transmit the supervised message cyclically, then stop it |
...
Work through these in order — each one builds on the previous, and the
screenshots form the documentation of the exercise.
...
5. **Inject the fault.** Stop the IG block so the message disappears from the
   bus. Wait at least the supervision timeout **plus** the debounce time, then
   read the status again. Bit 0 (`testFailed`) is now set. Screenshot it and
   note why it changed.
...
7. **Heal the fault.** Restart the cyclic transmission in CANalyzer and read the
   ...
   conditions met after the fault was confirmed. A confirmed DTC is not cleared
   immediately when the fault disappears; this retention is by design.
...
- **Reading the status too early.** Right after you stop the message, the
  supervision timeout and debounce haven't elapsed yet, so the fault still
  reads as absent. Wait for the timeout plus maturation time
...
!!! success "Key takeaways"
    - Missing-message supervision is timeout-based: the receiver detects when
      an expected cyclic frame stops arriving.
    - In CANalyzer, the Interactive Generator is the mechanism for injecting
      and healing the fault by controlling the supervised message.
    - The DTC status byte reflects the fault life cycle: `testFailed` for the
      current state, `confirmedDTC` for storage in error memory, healing, and
      re-evaluation at every driving cycle.
    - Debounce times and the power latch time must be observed for the
      exercise results to be reproducible.
    - Every observed status byte must be justified against the CDD
      specification.
