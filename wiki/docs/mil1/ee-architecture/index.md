...
|---|---|
| EEA standards | Engineering standards: EMC (electromagnetic compatibility), electrical systems, networks, diagnostics, vehicle configuration management, development processes |
...
| Network management | Wake-up and sleep management of the networks (related to AUTOSAR, see below) |
...
| Network database | CAN and LIN message IDs, data length, timing, and signal content/location — the DBC/LDF files you will use daily (see [CAN, LIN & Ethernet](../can-lin/index.md)) |
...
4. **AUTOSAR** (AUTomotive Open System ARchitecture). The open, standardized
   automotive software architecture makes software exchangeable across vehicle
   platforms, supplier solutions and OEM applications. Main benefits: a
   formally defined architecture, hardware independence, standardized software
   interfaces and functional-safety support.

## Vehicle Functions (VF)

A **Vehicle Function** is any functionality that requires several ECUs to
interact — usually over the CAN/LIN networks. Adaptive cruise control is a
classic example: radar, engine, brakes and steering all have to cooperate. VFs
carry the high-level integration requirements between vehicle ECUs and are
**specific to each car line and each E/E architecture**. They are documented in
detail (signals exchanged, algorithms, conditions) in the functional
specifications you will study in the [VF](../../mil3/vf/index.md) lessons.
...
in topology and in how they scale:
...
("PROXI" is the coded configuration string that describes which options a
specific vehicle was built with; storing it directly in the ECM means the
engine controller knows the vehicle's equipment without asking over the
network.)

Two recurring building blocks deserve attention:

- The **IPC** (Instrument Panel Cluster — the dashboard display) traditionally
  bridges the body and powertrain domains by sitting on both B-CAN and C-CAN;
  on the BEV architecture it moves to C-CAN only.
- The **Secure Gateway (SGW)** is the cyber-security checkpoint: diagnostic
  ...
  powertrain buses directly. When your diagnostic tool "can't see" an ECU, the
  SGW is often the reason.
...
(radio-frequency hub — key fob receiver), **ABS** (anti-lock braking), **ACC**
(adaptive cruise control), **TBM** (telematic box), **ECM** (engine control),
**CDCM** (chassis domain control) and the **EOBD** diagnostic socket.
Automatic-transmission variants (**GME AT**) add the **TCM** (transmission
control), **DTCM** (all-wheel-drive driveline control) and **AGSM** (gear
shifter module) to the same bus.

### CAN-C2 — chassis and driver assistance

CAN-C2 collects the chassis-area ECUs: **ABS**, **BCM**, **ESL** (electronic
steering lock), **PAM** (parking aid), **EPS** (electric power steering),
**HALF** (haptic lane feedback — the steering wheel that vibrates when you
drift lanes), **ORC**, **TBM**, **AAML/AAMR** (left/right active aerodynamics
actuators), **AFLS** (adaptive front lights), **CDCM**, **TVM** (torque
vectoring differential), plus the **J001** junction connectors and the EOBD
socket.

### CAN-BH — interior comfort

CAN-BH serves comfort and infotainment: **IPC**, **CSWM** (seat/steering-wheel
memory and heating), **LBSS/RBSS** (left/right blind-spot sensors), **AMP**
(Hi-Fi amplifier), **TTM/TTEBM** (trailer tow), **HVAC** (heating, ventilation
and air conditioning), **ETM** (infotelematics), **BCM**, **TBM**, **EMCM**
(the rotary infotainment selector), **ANC** (active/engine-sound enhancement)
...
switch and brings the networks back to life so the vehicle is ready (interior
lights, cluster, locks) before you even insert the key.
...
diagnostic connector — with the vehicle asleep or powered off:
